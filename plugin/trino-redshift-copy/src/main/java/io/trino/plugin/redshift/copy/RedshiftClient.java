...
import static io.trino.plugin.redshift.copy.RedshiftErrorCode.REDSHIFT_INVALID_TYPE;
import static io.trino.plugin.redshift.copy.RedshiftSessionProperties.isBatchedInsertsCopyEnabled;
        ...
        extends BaseJdbcClient
{
    ...
    public void finishInsertTable(ConnectorSession session, JdbcOutputTableHandle handle, Set<Long> pageSinkIds)
    {
        if (isBatchedInsertsCopyEnabled(session)) {
            Location copyLocationWithPrefix = outputPrefix(session, redshiftConfig.getBatchedInsertsCopyLocation());
    ...
    protected static Location outputPrefix(ConnectorSession session, Optional<String> copyLocation)
    {
